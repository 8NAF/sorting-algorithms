...
namespace mak
{
	namespace concepts
	{
		...
		concept tag_to_range =
			...
			(
				std::same_as<tag_t, std::contiguous_iterator_tag> ? std::ranges::contiguous_range<iterator_t> :
				std::same_as<tag_t, std::random_access_iterator_tag > ? std::ranges::random_access_range<iterator_t> :
